...
export const QuizExpanded = ({
    ...
    resetView,
    switchEdit
}: {quiz:Quiz;editQuiz:(id:number, newQuiz:Quiz)=> void;resetView:()=>void;switchEdit:()=> void;}) => {
    ...
    const editQuestionSub = (questionId: number, sub: string) => {
        editQuiz(quiz.id, {
            ...quiz,
            questionList: quiz.questionList.map((q: Question):Question => questionId === q.id ? {...q, submission:sub} : q)
        }
        );
    };
